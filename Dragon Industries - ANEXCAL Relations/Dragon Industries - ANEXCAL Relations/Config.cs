	...
	public class Config<E>
	{
		...
		*/
		...
		public void load(bool force = false) {
			...
			if (File.Exists(path)) {
				TTUtil.log("Loading config file at "+path, owner);
				try {
					...
					foreach (XmlNode e in root.ChildNodes) {
						...
						try {
							...
							if (!entry.validate(ref get)) {
								TTUtil.log("Chosen "+name+" value ("+raw+") was out of bounds, clamped to "+get, owner);
							}
							data[name] = get;
					...
					TTUtil.log("Config successfully loaded: "+vals, owner);
					if (missing.Count > 0) {
						string keys = string.Join(";", missing.ToArray());
						TTUtil.log("Note: "+missing.Count+" entries were missing from the config and so stayed the default values.", owner);
						TTUtil.log("Missing keys: "+keys, owner);
				...
				TTUtil.log("Config file does not exist at "+path+"; generating.", owner);
			...
			try {
				...
				foreach (E key in Enum.GetValues(typeof(E))) {
					try {
						...
						TTUtil.log("Could not generate XML node for "+key+": "+e.ToString(), owner);
					}
				}
				doc.Save(path);
				...
				TTUtil.log("Config failed to generate: "+ex.ToString(), owner);
			}
		}
